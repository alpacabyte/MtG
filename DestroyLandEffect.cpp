...
void DestroyLandEffect::UseEffect(std::shared_ptr<Player> activePlayer, std::shared_ptr<Player> otherPlayer, std::string cardName) {
    std::cout << "Your opponent's land card(s) that you can destroy: " << std::endl;
    int i = 0;
    ...
    int input = 9;
    while(!(std::cin >> input) || input >= otherPlayer->GetInPlay().size() || otherPlayer->GetInPlay().at(input)->GetType() != "Land"){
        std::cin.clear();
